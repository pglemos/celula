...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-white/65 backdrop-blur-xl text-card-foreground flex flex-col gap-6 rounded-[28px] border border-white/50 py-6 shadow-[0_4px_20px_rgba(0,0,0,0.03)] transition-all duration-300",
        className
      )}
      {...props}
    />
