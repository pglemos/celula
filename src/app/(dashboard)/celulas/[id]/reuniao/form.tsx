...
import { ArrowLeft, Check, Star, Send, Loader2 } from "lucide-react";
...
export function MeetingForm({ cellId, cellName, leaderId, members }: MeetingFormProps) {
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            formData.set("submitted_by", leaderId || members[0]?.id || "");
            formData.set("meeting_date", new Date().toISOString().split("T")[0]);
            formData.set("gods_presence", String(godsPresence));
            formData.set("decisions_for_christ", String(decisions));
            formData.set("theme", theme);
            formData.set("observations", notes);

            const attendanceData = members.map((m) => ({
                person_id: m.id,
                present: !!attendance[m.id],
            }));
            formData.set("attendance", JSON.stringify(attendanceData));
    ...
    return (
        <div className="space-y-6 max-w-2xl">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {cellName} • {new Date().toLocaleDateString("pt-BR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                    </p>
                </div>
            </div>
            ...
            <Card className="glass-card border-border/50 animate-fade-in-up">
                <CardHeader className="pb-3">
                    <CardTitle className="text-base flex items-center justify-between">
                        <span>✅ Presença</span>
                        <span className="text-sm font-normal text-muted-foreground">{presentCount}/{members.length}</span>
                    </CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                    {members.map((m) => (
                        ...
                        >
                            ...
                            {!attendance[m.id] && <span className="text-[10px] text-amber-400">⚠️ Ausente</span>}
                        </button>
                    ))}
                </CardContent>
            </Card>
            ...
            <Card className="glass-card border-border/50 animate-fade-in-up" style={{ animationDelay: "100ms" }}>
                <CardHeader className="pb-3"><CardTitle className="text-base">⭐ Presença de Deus</CardTitle></CardHeader>
                <CardContent>
                    <div className="flex gap-2">
                        {[1, 2, 3, 4, 5].map((rating) => (
                            <button key={rating} onClick={() => setGodsPresence(rating)} className="transition-transform hover:scale-110">
                                <Star className={cn("h-8 w-8 transition-colors", rating <= godsPresence ? "fill-amber-400 text-amber-400" : "text-muted-foreground/30")} />
                            </button>
                        ))}
                    </div>
                </CardContent>
            ...
            <Card className="glass-card border-border/50 animate-fade-in-up" style={{ animationDelay: "200ms" }}>
                <CardHeader className="pb-3"><CardTitle className="text-base">✝️ Decisões para Cristo</CardTitle></CardHeader>
                <CardContent>
                    <div className="flex items-center gap-4">
            ...
            <Card className="glass-card border-border/50 animate-fade-in-up" style={{ animationDelay: "300ms" }}>
                <CardContent className="p-5 space-y-4">
                    <div className="space-y-2">
                        ...
                        <Input placeholder="Ex: Amor ao próximo" value={theme} onChange={(e) => setTheme(e.target.value)} className="bg-secondary border-none" />
                    </div>
                    <div className="space-y-2">
                        <Label className="text-sm">📝 Observações</Label>
                        <Textarea placeholder="Observações sobre a reunião..." value={notes} onChange={(e) => setNotes(e.target.value)} className="bg-secondary border-none min-h-[80px]" />
                    </div>
                </CardContent>
            </Card>
            ...
            >
                {loading ? <><Loader2 className="h-5 w-5 animate-spin" />Enviando...</> : <><Send className="h-5 w-5" />Enviar Relatório</>}
            </Button>
        </div>
    );
}
